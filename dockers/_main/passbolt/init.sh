...
# Ensure sudo docker network
if ! sudo docker network ls | grep -q "$NETWORK_NAME"; then
  echo "→ Creating sudo docker network: $NETWORK_NAME"
  sudo docker network create "$NETWORK_NAME"
fi

echo "→ Starting Passbolt MariaDB..."
sudo docker compose up -d passbolt_db

# --- Wait for DB ---
echo "→ Waiting for database to be ready..."
until sudo docker compose exec passbolt_db mysqladmin ping \
...
sudo docker volume inspect passbolt_gpg >/dev/null 2>&1 || sudo docker volume create passbolt_gpg
sudo docker volume inspect passbolt_jwt >/dev/null 2>&1 || sudo docker volume create passbolt_jwt

# Start Passbolt
echo "🚀 Starting Passbolt service..."
sudo docker compose up -d passbolt

# --- Wait for container health ---
echo "→ Waiting for Passbolt to be healthy..."
while [ "$(sudo docker inspect -f '{{.State.Health.Status}}' passbolt_server)" != "healthy" ]; do
  sleep 2
...
if docker compose exec -T passbolt \
    su -m -s /bin/sh www-data -c \
    "/usr/share/php/passbolt/bin/cake passbolt show_user -u '${PASSBOLT_INIT_EMAIL}'" \
    2>/dev/null | grep -q "User found"; then
